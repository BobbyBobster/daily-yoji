...
def yoji(id_):
    with engine.connect() as conn:
        cols = ('id', 'kanji', 'reading', 'usage', 'meaning', 'is_wikipedia_link', 'sentence_id')
        query = text(f'''
...
def date_string(date):
    ...
    cols = ('id', 'kanji', 'reading', 'usage', 'meaning', 'is_wikipedia_link', 'sentence_id', 'date')
...
def translation(id_):
    with engine.connect() as conn:
        ...
        data = result.fetchall()
    return jsonify(
            dict(zip(('id', 'translation_text', 'owner'), data[0]))
            ), 200
