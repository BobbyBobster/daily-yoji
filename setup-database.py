...
def addSentences(cursor):
    ...
    for row in rows:
        ...
        tatoeba_response = requests.get(tatoeba_url_base + tatoeba_params + '&q=' + yoji)
...
def addLinks(cursor):
    ...
    with tempfile.NamedTemporaryFile(mode='wt', delete=False) as fp:
        ...
        wiki_yoji = subprocess.run(['zgrep', '-f', fp.name, 'data/jawiki-latest-all-titles-in-ns0.gz'], capture_output=True, text=True)
...
def addDates(cursor):
    ...
    datelist = [datetime.date.today() + datetime.timedelta(days=day) for day in range(rowcount)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--create', action='store_true', help='create tables in database')
    parser.add_argument('--populate', action='store_true', help='populate database with extracted data')
    parser.add_argument('--sentences', action='store_true', help='add example sentences from Tatoeba.org')
    parser.add_argument('--links', action='store_true', help='check for entries in Wikipedia and/or yoji-jukugo.com')
    parser.add_argument('--dates', action='store_true', help='add random datestamps to all rows')
